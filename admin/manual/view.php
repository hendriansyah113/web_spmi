<div class="table-responsive">
    <table class="table table-bordered">
        ...
        if (isset($_POST['search']) && $_POST['search'] == true) {
            $param = '%' . $keyword . '%';
            $sql = $pdo->prepare("SELECT * FROM kebijakan WHERE judul LIKE :judul OR deskripsi LIKE :deskripsi LIMIT " . $limit_start . "," . $limit);
            $sql->bindParam(':judul', $param);
            $sql->bindParam(':deskripsi', $param);
            $sql->execute();
            $sql2 = $pdo->prepare("SELECT COUNT(*) AS jumlah FROM kebijakan WHERE judul LIKE :judul OR deskripsi LIKE :deskripsi ");
            $sql2->bindParam(':judul', $param);
            $sql2->bindParam(':deskripsi', $param);
            $sql2->execute();
            $get_jumlah = $sql2->fetch();
        } else {
            $sql = $pdo->prepare("SELECT * FROM kebijakan LIMIT " . $limit_start . "," . $limit);
            $sql->execute();
            $sql2 = $pdo->prepare("SELECT COUNT(*) AS jumlah FROM kebijakan");
            $sql2->execute();
            $get_jumlah = $sql2->fetch();
        }
        ...
        while ($data = $sql->fetch()) {
            ...
            <tr>
                ...
                <td class="align-middle">
                    <a href="<?php echo $data['file_path']; ?>" class="btn btn-info" target="_blank">Lihat Dokumen</a>
                </td>

                <td class="align-middle">
                    <a href='form_ubah.php?id=<?php echo $data['id']; ?>' class='btn btn-warning'>Ubah</a>
                    <a href='proses_hapus.php?id=<?php echo $data['id']; ?>' class='btn btn-danger' onclick="return confirm('Apakah Anda yakin ingin menghapus dokumen ini?')">Hapus</a>
                </td>
            </tr>
            <?php
            $no++;
