...
<form method="get" action="">
        ...
        <div class="col-sm-3 col-md-2" id="form-tahun">
            <div class="form-group">
                ...
                <select name="tahun" class="form-control">
                    <option value="">Pilih</option>
                    <?php
                    $query = "SELECT YEAR(tanggal_lahir) AS tahun FROM ilkomfitria4 GROUP BY YEAR(tanggal_lahir)"; // Tampilkan tahun sesuai di tabel transaksi
    ...
    <a href="index_export.php" class="btn btn-default">Reset Filter</a>
        <p> <a href="http://localhost/coba_kelurahan/login_php/penduduk" class="btn btn-default">Kembali</a> </p>
</form>
<hr/>
...
if (isset($_GET['filter']) && !empty($_GET['filter'])) { // Cek apakah user telah memilih filter dan klik tombol tampilkan
    ...
    if ($filter == '1') { // Jika filter nya 1 (per tanggal)
        ...
        echo '<b>Data Penduduk Tanggal ' . $tgl . '</b><br /><br />';
        echo '<a href="proses_export.php?filter=1&tanggal=' . $_GET['tanggal'] . '" class="btn btn-success btn-sm">Export Excel</a><br /><br />';

        $query = "SELECT * FROM ilkomfitria4 WHERE DATE(tanggal_lahir)='" . $_GET['tanggal'] . "'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
        ...
        $query = "SELECT * FROM ilkomfitria4 WHERE MONTH(tanggal_lahir)='" . $_GET['bulan'] . "' AND YEAR(tanggal_lahir)='" . $_GET['tahun'] . "'"; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
        ...
        $query = "SELECT * FROM ilkomfitria4 WHERE YEAR(tanggal_lahir)='" . $_GET['tahun'] . "'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
    }
} else { // Jika user tidak mengklik tombol tampilkan
    echo '<b>Semua Data Penduduk</b><br /><br />';
    echo '<a href="proses_export.php" class="btn btn-success btn-sm">Export Excel</a><br /><br />';

    $query = "SELECT * FROM ilkomfitria4 ORDER BY tanggal_lahir"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
}
?>
...
<div class="table-responsive">
    <table class="table table-bordered">
        ...
        if ($row > 0) {
            $no = 1; // Tambahkan variabel $no untuk memberikan nomor urut
            while ($data = $sql->fetch()) {
                $tgl_lahir = date('d-m-Y', strtotime($data['tanggal_lahir']));

                echo "<tr>";
                echo "<td>" . $no . "</td>"; // Tampilkan nomor urut
                echo "<td>" . (isset($data['nik']) ? $data['nik'] : '') . "</td>";
                echo "<td>" . (isset($data['nama']) ? $data['nama'] : '') . "</td>";
                echo "<td>" . $tgl_lahir . "</td>";
                echo "<td>" . (isset($data['jenis_kelamin']) ? $data['jenis_kelamin'] : '') . "</td>";
                echo "<td>" . (isset($data['telp']) ? $data['telp'] : '') . "</td>";
                echo "<td>" . (isset($data['alamat']) ? $data['alamat'] : '') . "</td>";
                echo "<td>" . (isset($data['agama']) ? $data['agama'] : '') . "</td>";
                echo "<td>" . (isset($data['pekerjaan']) ? $data['pekerjaan'] : '') . "</td>";
                echo "</tr>";
